...
class CoordinatorServiceImpl final : public csce438::CoordService::Service
{
public:
  ServersMap servers_status; // <cluster_id, server_id> --> server status
  std::mutex servers_status_mtx;
  std::chrono::seconds heartbeat_timeout{10};

  std::vector<SyncInfo> syncs_status;
  std::mutex syncs_status_mtx;

  // recieve the heartbeat package from server
  Status
  Heartbeat(ServerContext *context, const ServerInfo *server_info, Confirmation *confirmation) override
  {
    // Your code here

    std::cout << "Received Heartbeat from server: " << server_info->server_id()
              << "  <" << server_info->server_ip() + ":" + server_info->server_port() << ">  " << server_info->type() << std::endl;

    if (server_info->type() == "synchronizer")
    {
      std::lock_guard<std::mutex> lock(syncs_status_mtx);
      syncs_status.push_back({server_info->server_ip(), server_info->server_port(), server_info->server_id()});
    }
    else
    {
  ...
  Status
  GetAllFollowerServers(ServerContext *context, const ID *request, ServerList *response) override
  {
    std::lock_guard<std::mutex> lock(syncs_status_mtx);
    for (const auto sync : syncs_status)
    {
      response->add_hostname(sync.ip);
      response->add_port(sync.port);
      response->add_serverid(sync.id);
    }
    return Status::OK;
  }
